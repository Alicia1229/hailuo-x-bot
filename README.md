...
- 飞书会先发 Hailuo 主卡片：Views Top 5、所有命中帖子表格链接、Related 高频词云、舆情正负面占比、热议话题 Top 3 和风险监控；Views Top 5 默认排除 `MiniMax_AI`、`Hailuo_AI`、`TopviewAIhq` 等企业官方账号，可用 `TOP_TWEET_EXCLUDED_AUTHORS` 调整；配置 `OPENAI_API_KEY` 后，舆情占比、热议话题和风险监控都会由 AI 总结/判定，可用 `OPENAI_BASE_URL` 切到 Mafia / OpenAI 兼容接口，竞品横向对比会在抓取完成后单独发第二张卡片
